...
final class ViewController: UIViewController, WKScriptMessageHandler, WKNavigationDelegate, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        case "connectMidiInput": if let id = message.body as? String, let endpoint = id.split(separator: ":").last.flatMap({ UInt32($0) }) { connect(endpoint: MIDIEndpointRef(endpoint)) }
        case "pairBleMidi":
    ...
    private func sendStatus(kind: String, ready: Bool, text: String) { let payload: [String: Any] = ["kind": kind, "ready": ready, "text": text]; sendJS("window.__iosHardwareCallback && window.__iosHardwareCallback(\(json(payload)))") }
